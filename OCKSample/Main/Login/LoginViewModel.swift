	...
	*/
	func signup(
		...
		do {
			...
			Logger.login.debug("Parse signup successful: \(user)")
			...
			default:
				Logger.login.error("""
					*** Error signing up for Parse Server. Are you running parse-hipaa and is the initialization \
					complete? Check http://localhost:1337 in your browser. For help, visit:
	...
	*/
	func loginAnonymously() async {
		do {
			...
			Logger.login.debug("Parse login anonymous successful: \(user)")
